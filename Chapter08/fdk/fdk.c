...
void back_projection(float *fp_h, short int* CT_numbers_h, int view_start, int view_end, int X_SIZE, int Y_SIZE, int Z_SIZE)
{
	float x,y,z,t,s,p_prime, ksi_prime,SO_s,factor,belta_rad,cos_belta,sin_belta;
	float m_f,m_z,n_f,n_y,temp;
	int m_less,n_less, i, j ,k, l;
	short int CT_number;
	float reco;
#pragma acc kernels copyin(fp_h[0:X_SIZE*Y_SIZE*Z_SIZE]) \
		    copyout(CT_numbers_h[0:RECONSIZE_Z*RECONSIZE*RECONSIZE])
{
   #pragma acc loop collapse(3) independent 
	for(i=0;i<RECONSIZE_Z;i++)  
	{
		for(j=0;j<RECONSIZE;j++)  
		{
			for(k=0;k<RECONSIZE;k++)  
			{
		z=(Z_CENTER-zstart+i)*recon_step_z; // if it is minus it statrs from zstart and goes down.
			y=(j-REC_XY_CENTER)*recon_step;
				x=(k-REC_XY_CENTER)*recon_step;
				 // initialize the reconstruted data
				reco = 0;
				for(l=view_start;l<view_end;l++)  
				{
					...
					temp=(1-m_z)*(1-n_y)*fp_h[l*Y_SIZE*Z_SIZE+m_less*Z_SIZE+n_less]+m_z*(1-n_y)*fp_h[l*Y_SIZE*Z_SIZE+(m_less+1)*Z_SIZE+n_less]+
						(1-m_z)*n_y*fp_h[l*Y_SIZE*Z_SIZE+m_less*Z_SIZE+n_less+1]+m_z*n_y*fp_h[l*Y_SIZE*Z_SIZE+(m_less+1)*Z_SIZE+n_less+1];
                     reco+=factor*temp;
				}   // end of belta--viewend
				
                temp=reco*4*PI/num_belta;
				if(temp<0) 
					temp=0.0;
				CT_number=(short int)((temp-water)/water*1000);
				//fwrite(&CT_number,sizeof(short int),1,ptr_ct);
				CT_numbers_h[i*RECONSIZE*RECONSIZE + j*RECONSIZE + k] = CT_number;
			}  // end of k-x
		}  // end of j-y
	}
  }
...
int main(int argc, char** argv)  
{
	...
	back_projection(fp_h, CT_numbers_h, view_start, view_end, X_SIZE, Y_SIZE, Z_SIZE);
